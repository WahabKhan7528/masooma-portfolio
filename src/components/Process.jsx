...
const Process = () => {
    ...
    useGSAP(() => {
        gsap.from('.proc-char', {
            y: 100, opacity: 0, rotateX: -90, stagger: 0.03,
            duration: 1.2, ease: 'power4.out',
            scrollTrigger: { trigger: '.process-title', start: 'top 88%' }
        });

        gsap.from('.process-sub', {
            opacity: 0, y: 20, duration: 1, delay: 0.3, ease: 'power3.out',
            scrollTrigger: { trigger: '.process-title', start: 'top 88%' }
        });
        ...
        gsap.from('.process-bar', {
            scaleX: 0, transformOrigin: 'left center', stagger: 0.15,
    ...
    return (
        ...
        >
            ...
            {/* Cards Grid */}
            <div className="process-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-dark-text/5 rounded-2xl border border-dark-text/5" style={{ perspective: '1000px' }}>
                {steps.map((step, i) => (
                    <div
                        key={step.num}
                        ref={el => cardRefs.current[i] = el}
                        className="process-card relative flex flex-col p-6 sm:p-8 bg-dark-bg cursor-default transition-colors duration-500 group hover:bg-dark-text/[0.03]"
                        style={{ transformStyle: 'preserve-3d' }}
                        onMouseEnter={() => setActiveStep(i)}
                        onMouseMove={(e) => handleCardMouseMove(e, i)}
                        onMouseLeave={() => { setActiveStep(null); handleCardMouseLeave(i); }}
                    >
                        {/* Glow overlay */}
                        <div
                        ...
                        <div className={`process-bar absolute top-0 left-0 right-0 h-[2px] transition-colors duration-500 ${activeStep === i ? 'bg-accent-violet' : 'bg-dark-text/10'}`} />
                        ...
                        <div className="flex items-center justify-between mb-8 sm:mb-12">
                            ...
                            <span className={`font-body text-[9px] font-semibold tracking-[2px] uppercase px-2.5 py-1 rounded-full border transition-all duration-300 ${activeStep === i ? 'border-accent-violet text-accent-violet' : 'border-dark-text/10 text-dark-text/30'}`}>
                                {step.tag}
                            </span>
                        </div>
